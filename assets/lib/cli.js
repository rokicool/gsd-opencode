...
export class CliFormatter {
  ...
  formatHelp() {
    ...
    lines.push('  node translate.js <config-file-1> [config-file-2] [...] [options]');
    lines.push('');
    ...
    lines.push('');
    lines.push(this.colorize('Multiple configs:', 'bright'));
    lines.push('  You can specify multiple config files. They will be merged in order:');
    lines.push('  - rules are concatenated from all configs');
    lines.push('  - include/exclude patterns are merged and deduplicated');
    lines.push('  - later configs override earlier ones for other properties');
    lines.push('');
