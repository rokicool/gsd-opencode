...
Use this procedure when `npx gsd-opencode@latest` is unavailable — e.g. during a publish outage or if you are working directly from the source repo.

## Prerequisites

- Node.js installed
- This repo cloned locally (`git clone https://github.com/rokicool/gsd-opencode`)
...
- `$HOME/.config/opencode/get-shit-done/` — workflows, references, templates
- `$HOME/.config/opencode/commands/gsd/` — slash commands
- `$HOME/.config/opencode/agents/gsd-*.md` — GSD agents
- `$HOME/.config/opencode/hooks/dist/` — compiled hooks
