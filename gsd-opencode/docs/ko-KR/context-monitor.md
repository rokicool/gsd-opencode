...
에이전트의 컨텍스트 윈도우 사용량이 높을 때 경고를 주는 post-tool 훅입니다 (OpenCode의 경우 `PostToolUse`, Gemini CLI의 경우 `AfterTool`).
...
두 훅 모두 `npx gsd-opencode` 설치 중에 자동으로 등록됩니다.
...
`$HOME/.config/opencode/settings.json`에 수동으로 등록하는 방법 (OpenCode):
