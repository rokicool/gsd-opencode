...
**생성 파일:** `PROJECT.md`, `REQUIREMENTS.md`, `ROADMAP.md`, `STATE.md`, `config.json`, `research/`, `AGENTS.md`
...
계획 수립 전 OpenCode의 예상 접근 방식을 미리 확인합니다.
...
OpenCode 세션 분석을 통해 8개 차원(커뮤니케이션 스타일, 의사결정 패턴, 디버깅 접근 방식, UX 선호도, 벤더 선택, 불만 유발 요인, 학습 스타일, 설명 깊이)으로 개발자 행동 프로필을 생성합니다. OpenCode의 응답을 개인화하는 아티팩트를 생성합니다.
...
- `AGENTS.md` 프로필 섹션 — OpenCode에 의해 자동으로 인식됩니다
