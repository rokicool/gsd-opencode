...
- REQ-NOTE-05: task, question, bash를 사용해서는 안 됩니다 — 인라인으로만 실행됩니다.
...
- REQ-SESSION-05: 모든 세션 작업은 `/new`(컨텍스트 초기화) 후에도 작동해야 합니다.
...
**명령어:** `/gsd-set-profile <simple|smart|genius|inherit>`
...
**목적:** OpenCode 세션 히스토리를 분석하여 8가지 차원에서 행동 프로파일을 구축하고, 개발자의 스타일에 맞게 OpenCode 응답을 개인화하는 산출물을 생성합니다.
...
- `AGENTS.md` 프로파일 섹션 — OpenCode가 자동으로 검색
...
- REQ-PROF-04: 생성된 산출물은 OpenCode가 검색할 수 있어야 합니다(AGENTS.md 통합).
...
OpenCode가 GSD 워크플로우 컨텍스트 밖에서 파일 편집을 시도하는 것을 감지하는 PreToolUse 훅입니다. 직접 편집 대신 `/gsd-quick` 또는 `/gsd-fast` 사용을 권고합니다. `hooks.workflow_guard`로 구성 가능합니다(기본값: false).
...
**요구사항.**
- REQ-MULTI-RT-01: 대화형 프롬프트는 다중 선택을 지원해야 합니다(예: OpenCode + Gemini).
...
### 68. OpenCode 스킬 마이그레이션

**대상:** `npx gsd-opencode`

**목적:** GSD 명령어를 하위 호환성을 유지하면서 OpenCode 2.1.88+ 스킬 형식으로 마이그레이션합니다.

**요구사항.**
- REQ-SKILLS-01: 설치 프로그램은 OpenCode 2.1.88+ 용 `skills/gsd-*/SKILL.md`를 작성해야 합니다.
- REQ-SKILLS-02: 설치 프로그램은 레거시 `commands/gsd/` 디렉토리를 자동 정리해야 합니다.
- REQ-SKILLS-03: Gemini 경로를 통해 이전 OpenCode 버전과의 하위 호환성을 유지해야 합니다.

**프로세스.**
1. **감지** — OpenCode 버전을 확인하여 스킬 지원 여부 판단
2. **마이그레이션** — 각 GSD 명령어에 대해 `skills/gsd-*/SKILL.md` 파일 작성
3. **정리** — 스킬이 설치되면 레거시 `commands/gsd/` 디렉토리 제거
4. **폴백** — 이전 OpenCode 버전을 위한 Gemini 경로 호환성 유지
...
**목적:** 파일이 편집 전에 읽혀지도록 하여 비-OpenCode 런타임에서의 무한 재시도 루프를 방지합니다.
