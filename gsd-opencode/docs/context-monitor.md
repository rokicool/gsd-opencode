...
A post-tool hook (`PostToolUse` for OpenCode, `AfterTool` for Gemini CLI) that warns the agent when context window usage is high.
...
Both hooks are automatically registered during `npx gsd-opencode` installation:
...
Manual registration in `$HOME/.config/opencode/settings.json` (OpenCode):
