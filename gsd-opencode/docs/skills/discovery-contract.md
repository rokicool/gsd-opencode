...
These roots are used for project-specific skills and for the project `AGENTS.md` skills section.
...
- `$HOME/.config/opencode/get-shit-done/skills/`
...
- `$HOME/.config/opencode/commands/gsd/`

This is not a skills root. Discovery code only checks whether it exists so inventory can report legacy OpenCode installs.
...
- Treat `$HOME/.config/opencode/get-shit-done/skills/` entries as deprecated/import-only.
- Treat `$HOME/.config/opencode/commands/gsd/` as legacy command installation metadata, not skills.
...
- Builds the project `AGENTS.md` skills section.
...
- Marks `legacy_claude_commands_installed` when `$HOME/.config/opencode/commands/gsd/` contains `.md` command files.
...
- `installation`: summary booleans for installed GSD skills and legacy OpenCode commands
