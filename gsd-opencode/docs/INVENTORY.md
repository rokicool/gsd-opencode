...
| `/gsd-ultraplan-phase` | [BETA] Offload plan phase to OpenCode's ultraplan cloud — drafts remotely, review in browser, import back via `/gsd-import`. OpenCode only. | [commands/gsd/ultraplan-phase.md](../commands/gsd/ultraplan-phase.md) |
...
| `/gsd-list-phase-assumptions` | Surface OpenCode's assumptions about a phase approach before planning. | [commands/gsd/list-phase-assumptions.md](../commands/gsd/list-phase-assumptions.md) |
...
| `/gsd-profile-user` | Generate developer behavioral profile and OpenCode-discoverable artifacts. | [commands/gsd/profile-user.md](../commands/gsd/profile-user.md) |
...
| `/gsd-set-profile` | Switch model profile for GSD agents (simple/smart/genius/inherit). | [commands/gsd/set-profile.md](../commands/gsd/set-profile.md) |
...
Full roster at `get-shit-done/workflows/*.md`. Workflows are thin orchestrators that commands reference internally; most are not read directly by end users. Rows below map each workflow file to its role (derived from the `<objective>` block) and, where applicable, to the command that invokes it.
...
| `list-phase-assumptions.md` | Surface OpenCode's assumptions about a phase before planning. | `/gsd-list-phase-assumptions` |
...
| `ultraplan-phase.md` | [BETA] Offload planning to OpenCode's ultraplan cloud; drafts remotely and imports back via `/gsd-import`. | `/gsd-ultraplan-phase` |
