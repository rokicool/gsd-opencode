...
- [Service] - [Endpoint: e.g., "/api/webhooks/stripe"]
  ...
  - Events: [e.g., "payment_intent.succeeded, customer.subscription.*"]
...
- External services code communicates with
- Authentication patterns (where secrets live, not secrets themselves)
...
- Note where secrets are managed (not what secrets are)
...
- Understanding data flow outside of application
- Setting up new environments
- Auditing third-party dependencies
- Planning for service outages or migrations

**Security note:**
Document WHERE secrets live (env vars, Vercel dashboard, 1Password), never WHAT secrets are.
</guidelines>
