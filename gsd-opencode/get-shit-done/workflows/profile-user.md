...
```bash
node $HOME/.config/opencode/get-shit-done/bin/gsd-tools.cjs generate-OpenCode-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.config/opencode/get-shit-done/bin/gsd-tools.cjs generate-OpenCode-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
