<purpose>
Verify phase goal achievement through goal-backward analysis. Check that codebase actually delivers what phase promised, not just that tasks were completed.
...
A task "create chat component" can be marked complete when component is a placeholder. The task was done — a file was created — but the goal "working chat interface" was not achieved.

Goal-backward verification starts from outcome and works backwards:
1. What must be TRUE for that goal to be achieved?
...
**Extract phase goal:** Parse ROADMAP.md for this phase's goal/description. This is outcome to verify, not tasks.
...
1. **State** goal: Take phase goal from ROADMAP.md
...
<!-- Goal-backward derivation expertise is baked into gsd-verifier agent -->
</step>
...
Check if component actually calls API:
...
- User flow completion (can you do full task?)
...
6. **Anti-Paterns Found:** Scan results table
