/**
 * set-profile-phase16.cjs — Switch profile in oc_config.json with three operation modes
 *
 ...
 *
 * Usage:
 *   node set-profile-phase16.cjs                      # Mode 1: validate current
 *   node set-profile-phase16.cjs genius               # Mode 2: switch to profile
 *   node set-profile-phase16.cjs 'custom:{...}'       # Mode 3: create profile
 *   node set-profile-phase16.cjs --dry-run genius     # Preview changes
 */
...
/**
 * Error codes for set-profile-phase16 operations
 */
const ERROR_CODES = {
...
function setProfilePhase16(cwd, args) {
  ...
  if (profileArgs.length > 1) {
    error('Too many arguments. Usage: set-profile-phase16 [profile-name | profileName:JSON] [--dry-run]', 'INVALID_ARGS');
  }
