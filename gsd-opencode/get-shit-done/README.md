...
That's it. Verify with `/gsd:help` inside your OpenCode interface.

<details>
<summary><strong>Non-interactive Install (Docker, CI, Scripts)</strong></summary>

```bash
npx get-shit-done-cc --global   # Install to ~/.config/opencode/
npx get-shit-done-cc --local    # Install to .opencode/
```
...
If file reads fail with tilde paths (`~/.config/opencode/...`), set `CLAUDE_CONFIG_DIR` before installing:
```bash
...
**OpenCode is powerful. GSD makes it reliable.**
