...
These bash commands are used throughout for hierarchical model discovery:

```bash
# Get all unique providers
opencode models 2>/dev/null | cut -d'/' -f1 | sort -u

# Get model count for a provider
opencode models 2>/dev/null | grep "^${provider}/" | wc -l

# Check if provider has subproviders (returns "true" or "false")
opencode models 2>/dev/null | grep "^${provider}/" | awk -F'/' '{print NF}' | head -1 | grep -q '^3$' && echo "true" || echo "false"

# Get unique subproviders for a provider
opencode models 2>/dev/null | grep "^${provider}/" | cut -d'/' -f2 | sort -u

# Get model count for a subprovider
opencode models 2>/dev/null | grep "^${provider}/${subprovider}/" | wc -l

# Get models for provider/subprovider (3-level)
opencode models 2>/dev/null | grep "^${provider}/${subprovider}/" | cut -d'/' -f3- | sort

# Get models for 2-level provider
opencode models 2>/dev/null | grep "^${provider}/" | cut -d'/' -f2- | sort

# Verify a model ID exists
opencode models 2>/dev/null | grep -q "^${model_id}$" && echo "valid" || echo "invalid"
```
...
question: "Which provider for planning agents?"
options:
  - label: "opencode"
    description: "OpenCode hosted models"
  - label: "openrouter"
    description: "OpenRouter multi-provider (150+ models)"
  - label: "xai"
    description: "xAI Grok models"
  - label: "synthetic"
    description: "Synthetic models via HuggingFace"
  - label: "back"
  ...
  - label: "zai-coding-plan"
    description: "Zai coding models"
```

Store selected provider as `quality_planning_provider`.

2. **Check for Subproviders**

Run: `opencode models | grep "^${quality_planning_provider}/" | awk -F'/' '{print NF}' | head -1 | grep -q '^3$' && echo "true" || echo "false"`
...
```
header: "{quality_planning_provider} Subprovider"
question: "Which subprovider?"
options:
  - label: "{subprovider1}"
    description: "{model_count} models"
  - label: "{subprovider2}"
    description: "{model_count} models"
  [all unique subproviders for this provider]
```
...
```bash
MODELS=$(opencode models 2>/dev/null | grep "^${quality_planning_provider}/${quality_planning_subprovider}/" | cut -d'/' -f3- | sort)
```

For 2-level structure (provider/model):
```bash
MODELS=$(opencode models 2>/dev/null | grep "^${quality_planning_provider}/" | cut -d'/' -f2- | sort)
```
...
question: "Which provider for execution agents?"
options:
...
Use Question tool with smart proposals:

```
header: "Quality Profile - Verification"
question: "Which provider for verification agents?"
options:
  - label: "Same as planning"
    description: "Use {quality_planning_model}"
...
Fetch all providers:
```bash
PROVIDERS=$(opencode models 2>/dev/null | cut -d'/' -f1 | sort -u)
```

For each provider, get model count:
```bash
COUNT=$(opencode models 2>/dev/null | grep "^${provider}/" | wc -l)
```

Use Question tool:

```
header: "Choose LLM Provider"
...
3. **Check for Subproviders**

Run: `opencode models 2>/dev/null | grep "^${overrideProvider}/" | awk -F'/' '{print NF}' | head -1 | grep -q '^3$' && echo "true" || echo "false"`

If result is "true" (provider has subproviders):

Fetch subproviders:
```bash
SUBPROVIDERS=$(opencode models 2>/dev/null | grep "^${overrideProvider}/" | cut -d'/' -f2 | sort -u)
```

For each subprovider, get model count:
```bash
COUNT=$(opencode models 2>/dev/null | grep "^${overrideProvider}/${subprovider}/" | wc -l)
```

Use Question tool:

```
header: "{overrideProvider} Subprovider"
question: "Which subprovider?"
options:
  - label: "{subprovider1}"
    description: "{model_count} models"
  - label: "{subprovider2}"
    description: "{model_count} models"
  [all subproviders for this provider]
  - label: "Cancel"
...
For 3-level structure (provider/subprovider/model):
```bash
MODELS=$(opencode models 2>/dev/null | grep "^${overrideProvider}/${overrideSubprovider}/" | cut -d'/' -f3- | sort)
```

For 2-level structure (provider/model):
```bash
MODELS=$(opencode models 2>/dev/null | grep "^${overrideProvider}/" | cut -d'/' -f2- | sort)
```
