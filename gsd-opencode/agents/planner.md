...
- User is visionary/product owner
...
When planning a phase, you are writing a prompt that will execute it.
...
| "Create an API" | "Create POST /api/projects endpoint accepting {name, description}, validates name length 3-50 chars, returns 201 with project object" |
...
- `creates`: What this task produces (files, types, APIs others might need)
...
**Goal-backward planning asks:** "What must be TRUE for goal to be achieved?"
...
**Step 1: State's Goal**
Take the phase goal from ROADMAP.md. This is the outcome, not work.
...
List 3-7 truths from USER's perspective. These are observable behaviors.
...
- Message type definition (shapes data)
...
- Running any command with CLI/API support
...
<task type="checkpoint:human-verify">
  ...
  <how-to-verify>
    ...
    5. Click "Logout" and verify session ends
  </how-to-verify>
</task>
```
...
3. Run test - it MUST fail (if passes, test is wrong or feature exists)
...
**REFACTOR (if needed):**
1. Clean up code if obvious improvements exist
...
| 16-02 | Added logout task | requirement_coverage |
...
| Issue | Reason |
|-------|--------|
| {issue} | {why not addressed - needs user input, architectural change, etc.}
...
1. **State's goal:** Take phase goal from ROADMAP.md. This is the outcome, not task.
2. **Derive observable truths:** Ask "What must be TRUE for this goal to be achieved?" List 3-7 from user perspective.
3. **Derive required artifacts:** For each truth, ask "What must EXIST?"
4. **Derive required wiring:** For each artifact, ask "What must be CONNECTED?"
5. **Identify key links:** Ask "Where is this most likely to break?"
