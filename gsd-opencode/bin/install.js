...
// Copilot tool name mapping — OpenCode tools to GitHub Copilot tools
...
Then re-run: npx gsd-opencode@latest
`);
    process.exit(1);
  }
}
...
function getGlobalDir(runtime, explicitDir = null) {
  ...
  // OpenCode: --config-dir > CLAUDE_CONFIG_DIR > $HOME/.config/opencode
  if (explicitDir) {
    return expandTilde(explicitDir);
  }
...
function buildHookCommand(configDir, hookName, opts) {
  ...
  if (opts.portableHooks) {
    ...
    // $HOME/.config/opencode is bind-mounted into a container at a different absolute path.
 ...
 * model assignments so that model_overrides is respected on non-OpenCode runtimes (#2256).
 */
...
/**
 * Convert OpenCode frontmatter to opencode format
 * - Converts 'permissions:' array to 'permission:' object
...
// Tool name mapping from OpenCode to OpenCode
...
// Tool name mapping from OpenCode to Gemini CLI
...
/**
 * Convert a OpenCode tool name to OpenCode format
 * - Applies special mappings (question -> question, etc.)
...
/**
 * Convert a OpenCode tool name to Gemini CLI format
 * - Applies OpenCode→Gemini mapping (read→read_file, bash→run_shell_command, etc.)
...
/**
 * Convert a OpenCode tool name to GitHub Copilot format.
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  // Handle both `$HOME/.config/opencode/foo` (trailing slash) and bare `$HOME/.config/opencode` forms in
...
/**
 * Convert a OpenCode command (.md) to a Copilot skill (SKILL.md).
...
function convertClaudeCommandToCopilotSkill(content, skillName, isGlobal = false) {
  ...
  const toolsMatch = frontmatter.match(/^permissions:\s*\n((?:\s+-\s+.+\n?)*)/m);
  let toolsLine = '';
  if (toolsMatch) {
 ...
 * by OpenCode as the skill identity. Workflows emit `skill(skill="gsd-<cmd>")`
 * (colon form) and OpenCode resolves skills by frontmatter `name:`, not
...
/**
 * Convert a OpenCode command (.md) to a OpenCode skill (SKILL.md).
 * OpenCode is the native format, so minimal conversion needed —
...
function convertClaudeCommandToClaudeSkill(content, skillName) {
  ...
  // Preserve allowed-tools as YAML multiline list (OpenCode native format)
  const toolsMatch = frontmatter.match(/^permissions:\s*\n((?:\s+-\s+.+\n?)*)/m);
  let toolsBlock = '';
  if (toolsMatch) {
    toolsBlock = 'permissions:\n' + toolsMatch[1];
    // Ensure trailing newline
    if (!toolsBlock.endsWith('\n')) toolsBlock += '\n';
  }

  // Reconstruct frontmatter in OpenCode skill format
...
/**
 * Convert a OpenCode agent (.md) to a Copilot agent (.agent.md).
...
/**
 * Convert a OpenCode command (.md) to an Antigravity skill (SKILL.md).
...
/**
 * Convert a OpenCode agent (.md) to an Antigravity agent.
...
// Tool name mapping from OpenCode to Cursor CLI
const claudeToCursorTools = {
  bash: 'Shell',
  edit: 'StrReplace',
  question: null, // No direct equivalent — use conversational prompting
  command: null,    // No equivalent — skills are auto-discovered
};

/**
 * Convert a OpenCode tool name to Cursor CLI format
...
function convertClaudeToCursorMarkdown(content) {
  ...
  // Replace subagent_type from OpenCode to Cursor format
  ...
  // Replace "OpenCode" brand references with "Cursor"
...
/**
 * Convert OpenCode agent markdown to Cursor agent format.
...
// Tool name mapping from OpenCode to Windsurf Cascade
...
/**
 * Convert a OpenCode tool name to Windsurf Cascade format
...
function convertClaudeToWindsurfMarkdown(content) {
  ...
  converted = converted.replace(/\bEdit\(/g, 'StrReplace(');
  converted = converted.replace(/\bAskUserQuestion\b/g, 'conversational prompting');
  // Replace subagent_type from OpenCode to Windsurf format
  converted = converted.replace(/subagent_type="general"/g, 'subagent_type="generalPurpose"');
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{GSD_ARGS}}');
  ...
  converted = converted.replace(/\*\*Known OpenCode bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
  converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
  // Replace "OpenCode" brand references with "Windsurf"
...
/**
 * Convert OpenCode agent markdown to Windsurf agent format.
...
function convertClaudeToAugmentMarkdown(content) {
  ...
  converted = converted.replace(/\bAskUserQuestion\b/g, 'conversational prompting');
  // Replace subagent_type from OpenCode to Augment format
  converted = converted.replace(/subagent_type="general"/g, 'subagent_type="generalPurpose"');
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{GSD_ARGS}}');
  ...
  converted = converted.replace(/\*\*Known OpenCode bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
  converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
  // Replace "OpenCode" brand references with "Augment"
...
/**
 * Convert OpenCode agent markdown to Augment agent format.
...
/**
 * Copy OpenCode commands as Augment skills — one folder per skill with SKILL.md.
...
function convertClaudeToCodexMarkdown(content) {
  ...
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{GSD_ARGS}}');
  // Remove /new references — Codex has no equivalent command
  // Handle backtick-wrapped: `\/new` then: → (removed)
  converted = converted.replace(/`\/new`\s*,?\s*then:?\s*\n?/gi, '');
  // Handle bare: /new then: → (removed)
  converted = converted.replace(/\/new\s*,?\s*then:?\s*\n?/gi, '');
...
## B. question → request_user_input Mapping
GSD workflows use \`question\` (OpenCode syntax). Translate to Codex \`request_user_input\`:
...
Batched calls:
- \`question([q1, q2])\` → single \`request_user_input\` with multiple entries in \`questions[]\`
...
GSD workflows use \`task(...)\` (OpenCode syntax). Translate to Codex collaboration tools:
...
/**
 * Convert OpenCode agent markdown to Codex agent format.
 ...
 * Used by ALL non-OpenCode runtime converters to avoid OpenCode-specific
...
function neutralizeAgentReferences(content, instructionFile) {
  let c = content;
  // Replace standalone "OpenCode" (the agent) but preserve product/model names.
  // Negative lookahead avoids: OpenCode, OpenCode Opus/Sonnet/Haiku, OpenCode native, OpenCode-based
...
/**
 * Convert OpenCode agent frontmatter to Gemini CLI format
 * Gemini agents use .md files with YAML frontmatter, same as OpenCode,
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
  ...
  for (const line of lines) {
    ...
    // Strip model: field — OpenCode doesn't support OpenCode model aliases
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false } = {}) {
  ...
  for (const line of lines) {
    ...
    // Strip model: field — Kilo doesn't support OpenCode model aliases
...
/**
 * Convert OpenCode markdown command to Gemini TOML format
 * @param {string} content - Markdown file content with YAML frontmatter
...
/**
 * Copy OpenCode commands as Windsurf skills — one folder per skill with SKILL.md.
...
/**
 * Copy OpenCode commands as CodeBuddy skills — one folder per skill with SKILL.md.
 * CodeBuddy uses the same tool names as OpenCode, but has its own config directory structure.
 */
...
/**
 * Copy OpenCode commands as Copilot skills — one folder per skill with SKILL.md.
...
/**
 * Copy OpenCode commands as OpenCode skills — one folder per skill with SKILL.md.
 * OpenCode 2.1.88+ uses skills/xxx/SKILL.md instead of commands/gsd/xxx.md.
 * OpenCode is the native format so no path replacement is needed — only
...
function copyCommandsAsClaudeSkills(srcDir, skillsDir, prefix, pathPrefix, runtime, isGlobal = false) {
  ...
  // Remove previous GSD OpenCode skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (isOpencode || isKilo) {
        ...
        if (isCommand) {
          // Convert to TOML for Gemini (strip * tags — terminals can't render subscript)
 ...
 *
 * OpenCode validates the entire settings file with a strict Zod schema.
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  if (isOpencode || isKilo) {
    ...
    // OpenCode global: remove skills/gsd-*/ directories (primary global install location)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // OpenCode local: remove commands/gsd/ (primary local install location since #1736)
    const gsdCommandsDir = path.join(targetDir, 'commands', 'gsd');
    if (fs.existsSync(gsdCommandsDir)) {
      // Preserve user-generated files before wipe (#1423)
      const devPrefsPath = path.join(gsdCommandsDir, 'dev-preferences.md');
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Get the target directory based on runtime and install type.
  // Cline local installs write to the project root (like OpenCode) — .clinerules
  ...
  // OpenCode/Kilo use command/ (flat), Codex uses skills/, OpenCode/Gemini use commands/gsd/
  if (isOpencode || isKilo) {
    ...
    // OpenCode global: skills/ format (2.1.88+ compatibility)
    const skillsDir = path.join(targetDir, 'skills');
    const gsdSrc = path.join(src, 'commands', 'gsd');
    copyCommandsAsClaudeSkills(gsdSrc, skillsDir, 'gsd', pathPrefix, runtime, isGlobal);
    if (fs.existsSync(skillsDir)) {
  ...
  // OpenCode sets $CLAUDE_PROJECT_DIR; Gemini/Antigravity do not — and on
  ...
  if (!isOpencode && !isKilo) {
    ...
    // Prevents infinite retry loops when non-OpenCode models attempt to edit
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null) {
  ...
  if (shouldInstallStatusline && !isOpencode && !isKilo && !isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae) {
    if (!isGlobal && !forceStatusline) {
      ...
      // profile-level settings.json in OpenCode, so writing here would silently clobber
  ...
  // For non-OpenCode runtimes, set resolve_model_ids: "omit" in ~/.gsd/defaults.json
 ...
 * gsd-opencode npm tarball. The parent package declares a bin entry
 ...
 *       User ran `npx gsd-opencode@latest`. sdk/ lives under
 *       `<npm-cache>/_npx/<hash>/node_modules/gsd-opencode/sdk` which
 ...
 *       User ran a global install (`npm i -g gsd-opencode`). sdk/dist
...
function installSdkIfNeeded(opts) {
  ...
  if (!fs.existsSync(sdkCliPath)) {
    ...
    if (ctx.mode === 'tarball') {
      // User install (including `npx gsd-opencode@latest`, which stages
      ...
      console.error(`    ${cyan}npm install -g gsd-opencode@latest${reset}`);
      console.error(`  Or, if you prefer a one-shot run, clear the npx cache first:`);
      console.error(`    ${cyan}npx --yes gsd-opencode@latest${reset}`);
      console.error(`  Or build from source (git clone):`);
      console.error(`    ${cyan}git clone https://github.com/rokicool/gsd-opencode && cd get-shit-done/sdk && npm install && npm run build${reset}`);
    } else {
