...
> **Comprehensive reference.** Core rules auto-load from `.opencode/rules/`. This document provides deep explanations and examples for when you need the full picture.

This document explains how GSD is written so future OpenCode instances can contribute consistently.

## Core Philosophy

GSD is a **meta-prompting system** where every file is both implementation and specification. Files teach OpenCode how to build software systematically. The system optimizes for:

- **Solo developer + OpenCode workflow** (no enterprise patterns)
- **Context engineering** (manage OpenCode's context window deliberately)
...
**Task types:**
- `type="auto"` — OpenCode executes autonomously
...
**@-references are lazy loading signals.** They tell OpenCode what to read, not pre-loaded content.
...
6. **Solo developer + OpenCode** — no enterprise patterns
