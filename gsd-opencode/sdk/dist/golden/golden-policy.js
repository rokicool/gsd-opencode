...
export function isMutationCanonicalCmd(canonical) {
    const spaced = canonical.replace(/\./g, ' ');
    for (const m of QUERY_MUTATION_COMMANDS) {
        if (m === canonical || m === spaced)
            return true;
    }
    return false;
}
const MUTATION_DEFERRED_REASON = 'Listed in QUERY_MUTATION_COMMANDS — mutates `.planning/`, git, or profile files. Subprocess golden vs gsd-tools.cjs is covered where a tmp fixture or `--dry-run` exists in golden.integration.test.ts; otherwise handler parity lives in sdk/src/query/*-mutation.test.ts, commit.test.ts, phase-lifecycle.test.ts, workstream.test.ts, intel.test.ts, profile.test.ts, template.test.ts, docs-init.ts, or uat.test.ts as applicable.';
/** Registry commands with no `gsd-tools.cjs` analogue — cannot have subprocess JSON parity. */
const NO_CJS_SUBPROCESS_REASON = {
    'phases.archive': 'No `gsd-tools.cjs` command for `phases archive` (SDK-only). Covered in sdk/src/query/phase-lifecycle.test.ts.',
    'check.config-gates': 'SDK-only decision-routing query (`.planning/research/decision-routing-audit.md` §3.3). Covered in sdk/src/query/config-gates.test.ts.',
    'check.phase-ready': 'SDK-only decision-routing query (audit §3.4). Covered in sdk/src/query/phase-ready.test.ts.',
    'route.next-action': 'SDK-only decision-routing query (audit §3.1). Covered in sdk/src/query/route-next-action.test.ts.',
    'check.auto-mode': 'SDK-only decision-routing query (audit §3.5). Covered in sdk/src/query/check-auto-mode.test.ts.',
    ...
    'phase.list-plans': 'SDK-only listing helper for agents (no `gsd-tools.cjs` mirror). Covered in sdk/src/query/phase-list-queries.test.ts.',
    'phase.list-artifacts': 'SDK-only artifact enumeration (no CJS mirror). Covered in sdk/src/query/phase-list-queries.test.ts.',
    'plan.task-structure': 'SDK-only structured plan parse (no CJS mirror). Covered in sdk/src/query/plan-task-structure.test.ts.',
    'requirements.extract-from-plans': 'SDK-only requirements aggregation (no CJS mirror). Covered in sdk/src/query/requirements-extract-from-plans.test.ts.',
};
const READ_HANDLER_ONLY_REASON = (cmd) => `No ` +
    ...
    cmd +
    '`.';
function buildIntegrationCoveredSet() {
    return new Set([
...
export const GOLDEN_PARITY_EXCEPTIONS = buildGoldenParityExceptions();
function buildGoldenParityExceptions() {
    const out = {};
    for (const c of getCanonicalRegistryCommands()) {
        if (GOLDEN_PARITY_INTEGRATION_COVERED.has(c))
            continue;
...
export function verifyGoldenPolicyComplete() {
    const canon = getCanonicalRegistryCommands();
    const missingException = [];
    for (const c of canon) {
        if (GOLDEN_PARITY_INTEGRATION_COVERED.has(c))
            continue;
        if (!Object.prototype.hasOwnProperty.call(GOLDEN_PARITY_EXCEPTIONS, c))
            missingException.push(c);
    }
...
//# sourceMappingURL=golden-policy.js.map