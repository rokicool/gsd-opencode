        ...
        try {
            ...
            if (!matched) {
                if (!queryFallbackToCjsEnabled()) {
                    throw new GSDError(`Unknown command: "${tokens.join(' ')}". Use a registered \`gsd-sdk query\` subcommand (see sdk/src/query/QUERY-HANDLERS.md) or invoke \`node …/gsd-tools.cjs\` for CJS-only operations. Set GSD_QUERY_FALLBACK=registered (default) to allow automatic fallback.`, ErrorClassification.Validation);
                }
                const { resolveGsdToolsPath } = await import('./gsd-tools.js');
                const gsdPath = resolveGsdToolsPath(args.projectDir);
                console.error(`[gsd-sdk] '${tokens.join(' ')}' not in native registry; falling back to gsd-tools.cjs.`);
