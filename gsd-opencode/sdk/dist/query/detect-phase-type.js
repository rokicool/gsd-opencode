...
async function roadmapHeadingForPhase(projectDir, phaseNum, workstream) {
...
export const detectPhaseType = async (args, projectDir, workstream) => {
    const raw = args[0];
    if (!raw) {
    ...
    if (phaseDirFull && existsSync(phaseDirFull)) {
        try {
            dirFiles = readdirSync(phaseDirFull, { recursive: false });
        }
        catch {
            dirFiles = [];
        }
...
//# sourceMappingURL=detect-phase-type.js.map