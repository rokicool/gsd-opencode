...
export function truncateContent(content, maxLen = 2000) {
    if (content.length <= maxLen)
        return content;
    return content.substring(0, maxLen) + '... [truncated]';
}
/** Line-delimited JSONL reader — same behavior as CJS `streamExtractMessages`. */
export async function streamExtractMessages(filePath, filterFn, maxMessages) {
...
export async function runExtractMessages(projectArg, options, overridePath) {
    ...
    if (!matchedDir) {
        ...
        if (matches.length === 1) {
            ...
            if (exactNameMatches.length === 1) {
                ...
                throw new GSDError(`Multiple projects match "${projectArg}":\n${names.join('\n')}\nBe more specific.`, ErrorClassification.Validation);
            }
        }
    }
    if (!matchedDir) {
        ...
        throw new GSDError(`No project matching "${projectArg}". Available projects:\n${available.join('\n')}`, ErrorClassification.Validation);
    }
    ...
    if (options.sessionId) {
        ...
        if (sessions.length === 0) {
            throw new GSDError(`Session "${options.sessionId}" not found in project "${projectName}".`, ErrorClassification.Validation);
        }
    }
...
//# sourceMappingURL=profile-extract-messages.js.map