 ...
 */
export const commit = async (args, projectDir, _workstream) => {
    const allArgs = [...args];
    // Extract flags
 ...
 */
export const checkCommit = async (_args, projectDir, _workstream) => {
    const paths = planningPaths(projectDir);
    let commitDocs = true;
    try {
        const raw = await readFile(paths.config, 'utf-8');
...
// ─── commitToSubrepo ─────────────────────────────────────────────────────
export const commitToSubrepo = async (args, projectDir, _workstream) => {
