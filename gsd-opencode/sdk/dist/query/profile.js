...
import { PROFILING_QUESTIONS, generateClaudeInstruction, isAmbiguousAnswer, } from './profile-questionnaire-data.js';
 ...
 */
export const learningsListHandler = async () => {
...
export const learningsCopy = async (_args, projectDir, workstream) => {
 ...
 */
function learningsPruneStore(olderThan) {
...
export const learningsPrune = async (args) => {
...
export const learningsDelete = async (args) => {
    const id = args[0];
    if (!id) {
...
export const extractMessages = async (args) => {
    ...
    const sessionIdx = args.indexOf('--session') !== -1 ? args.indexOf('--session') : args.indexOf('--session-id');
    const sessionId = sessionIdx !== -1 ? args[sessionIdx + 1] : null;
    const limitIdx = args.indexOf('--limit');
    const limit = limitIdx !== -1 ? (parseInt(args[limitIdx + 1], 10) || null) : null;
    const projectArg = args[0];
    if (!projectArg || projectArg.startsWith('--')) {
        throw new GSDError('Usage: gsd-tools extract-messages <project> [--session <id>] [--limit N] [--path <dir>]\nRun scan-sessions first to see available projects.', ErrorClassification.Validation);
    }
...
export const scanSessions = async (args) => {
    const pathIdx = args.indexOf('--path');
    const overridePath = pathIdx !== -1 ? args[pathIdx + 1] : null;
 ...
 */
export const profileSample = async (args) => {
    const pathIdx = args.indexOf('--path');
    const overridePath = pathIdx !== -1 ? args[pathIdx + 1] : null;
    const limitIdx = args.indexOf('--limit');
...
export const profileQuestionnaire = async (args, _projectDir) => {
    ...
    if (answerValues.length !== PROFILING_QUESTIONS.length) {
        throw new GSDError(`Expected ${PROFILING_QUESTIONS.length} answers (comma-separated), got ${answerValues.length}`, ErrorClassification.Validation);
    }
    const dimensions = {};
    const analysis = {
        profile_version: '1.0',
    ...
    for (let i = 0; i < PROFILING_QUESTIONS.length; i++) {
        ...
        if (!selectedOption) {
            throw new GSDError(`Invalid answer "${answerValue}" for ${question.dimension}. Valid values: ${question.options.map((o) => o.value).join(', ')}`, ErrorClassification.Validation);
        }
