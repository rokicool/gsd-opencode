...
function requiresIdCitation(decision) {
...
function decisionMentioned(haystack, decision) {
    ...
    if (idRe.test(haystack))
        return true;
    const phrase = softPhrase(decision.text);
    if (!phrase)
        return false; // too short to soft-match — id citation required
...
async function loadPlanSections(phaseDir) {
...
function planSectionsMention(planSections, decision) {
    for (const p of planSections) {
        if (decisionMentioned(p.designated, decision))
            return true;
    }
    return false;
}
async function loadGateConfig(projectDir, workstream) {
    try {
        ...
        if (typeof v === 'string') {
            ...
            if (lower === 'false' || lower === 'true')
                return lower !== 'false';
            console.warn(`[gsd] workflow.context_coverage_gate is a string "${v}" — expected boolean. Defaulting to ON.`);
            return true;
        }
        if (v !== undefined && v !== null) {
            console.warn(`[gsd] workflow.context_coverage_gate has invalid type ${typeof v} (value: ${JSON.stringify(v)}); expected boolean. Defaulting to ON.`);
        }
...
// ─── Plan-phase gate ──────────────────────────────────────────────────────
export const checkDecisionCoveragePlan = async (args, projectDir, workstream) => {
 ...
 */
async function recentCommitMessages(projectDir, limit = 200) {
    try {
...
async function readModifiedFilesContent(projectDir, summaries) {
        ...
        for (const blockMatch of blockMatches) {
            ...
            for (const f of files) {
                if (!f)
                    continue;
                if (total >= 50)
                    break; // cap total files across all summaries
                // Reject absolute paths AND any relative path that escapes projectDir.
                if (!isInsideRoot(f, projectDir)) {
                    console.warn(`[gsd] decision-coverage: skipping files_modified entry "${f}" — outside project root`);
                    continue;
                }
...
export const checkDecisionCoverageVerify = async (args, projectDir, workstream) => {
    const phaseDir = args[0] ? resolvePath(args[0], projectDir) : '';
    const contextPath = args[1] ? resolvePath(args[1], projectDir) : '';
    const enabled = await loadGateConfig(projectDir, workstream);
    if (!enabled) {
...
//# sourceMappingURL=check-decision-coverage.js.map