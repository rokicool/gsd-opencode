...
Note: 06-03/06-04 are gap-closure plans; ROADMAP Phase 06 may list fewer baseline plans.
...
<task type="auto">
  <name>Task 0: Confirm install entrypoints + enable question tool + enforce sync (command ↔ agents)</name>
  <files>
gsd-opencode/command/gsd/settings.md
gsd-opencode/command/gsd/set-profile.md
gsd-opencode/agents/gsd-settings.md
...
## Install mapping guard
- Confirm `gsd-opencode/bin/install.js` still maps `gsd-opencode/command/gsd/*` into the installed `.opencode/command/gsd/*` destination.
  - Why: if the install mapping is wrong, users will keep running stale prompts even if we fix the source files.
  - If this mapping is missing or incorrect, STOP and fix it via the Phase 06 gap-closure install plan (`06-03-PLAN.md`) before continuing.
...
## Tools enablement (interactive input)
- In all four prompt files, update frontmatter `tools:` blocks to explicitly enable interactive input by adding:
  - `question: true`
...
rg -n "command/gsd" gsd-opencode/bin/install.js

# Verify /gsd-set-profile prompt still uses per-profile overrides (not legacy global stage keys)
rg -n "profiles\.custom_overrides\.\{newProfile\}" gsd-opencode/command/gsd/set-profile.md
rg -n "profiles\.custom_overrides\.\{profile\}" gsd-opencode/command/gsd/set-profile.md
set +e
rg -n "profiles\.custom_overrides\.(planning|execution|verification)\b" gsd-opencode/command/gsd/set-profile.md
rc=$?
set -e
if [ "$rc" -eq 0 ]; then
...
# Positive assertion: stage keys live under the profile key
rg -n "profiles\.custom_overrides\.\{newProfile\}\.(planning|execution|verification)\b" gsd-opencode/command/gsd/set-profile.md

# Verify /gsd-settings uses per-profile overrides (not legacy global stage keys)
rg -n "profiles\.custom_overrides\.\{profile\}" gsd-opencode/command/gsd/settings.md

# Positive assertion: stage keys live under the profile key
rg -n "profiles\.custom_overrides\.\{profile\}\.(planning|execution|verification)\b" gsd-opencode/command/gsd/settings.md

set +e
rg -n "profiles\.custom_overrides\.(planning|execution|verification)\b" gsd-opencode/command/gsd/settings.md
rc=$?
set -e
if [ "$rc" -eq 0 ]; then
  echo "Legacy-only override path still present in settings.md"; exit 1
elif [ "$rc" -eq 1 ]; then
  :
else
  echo "rg error (rc=$rc)"; exit "$rc"
...
Manual spot-check (post-install, in any repo):
1) Run: `npx gsd-opencode --local`
2) Open the installed entrypoint files:
   - `./.opencode/command/gsd/settings.md`
   - `./.opencode/command/gsd/set-profile.md`
3) Confirm BOTH:
   - Frontmatter contains `tools:` with `question: true`
   - The installed file contents include the updated menu/legend requirements from `gsd-opencode/command/gsd/*.md`
  </verify>
  <done>
- `bin/install.js` still references `command/gsd` mapping (install uses command entrypoints).
- All four prompt files declare `question: true` in frontmatter.
- Both agent prompt copies exist and are identical to their corresponding command/ entrypoints.
  </done>
</task>

<task type="auto">
  <name>Task 1: Convert menus + confirm flows to question-tool selection (no freeform number/letter parsing)</name>
  <files>
gsd-opencode/command/gsd/settings.md
gsd-opencode/agents/gsd-settings.md
gsd-opencode/command/gsd/set-profile.md
...
# Prompts must enable question tool everywhere
rg -n "question:\s*true" gsd-opencode/command/gsd/settings.md gsd-opencode/command/gsd/set-profile.md gsd-opencode/agents/gsd-settings.md gsd-opencode/agents/gsd-set-profile.md

# Settings menu labels must exist (static contract check)
rg -n "Change active profile" gsd-opencode/command/gsd/settings.md
rg -n "Edit stage override" gsd-opencode/command/gsd/settings.md
rg -n "Clear stage override" gsd-opencode/command/gsd/settings.md

# Set-profile confirm/edit/cancel labels must exist (static contract check)
rg -n "Confirm change" gsd-opencode/command/gsd/set-profile.md
rg -n "Edit proposed stage models" gsd-opencode/command/gsd/set-profile.md
rg -n "\bCancel\b" gsd-opencode/command/gsd/set-profile.md

# Negative assertion: legacy freeform wording should not remain
set +e
rg -n "Type a number|Selection:\s*$|Confirm\?\s*\[y/N\]" gsd-opencode/command/gsd/settings.md gsd-opencode/command/gsd/set-profile.md
rc=$?
set -e
if [ "$rc" -eq 0 ]; then
  echo "Legacy freeform input wording still present"; exit 1
elif [ "$rc" -eq 1 ]; then
  :
else
  echo "rg error (rc=$rc)"; exit "$rc"
...
<task type="auto">
  <name>Task 2: Make '* = overridden' legend an unconditional required output line in /gsd-settings</name>
  <files>
gsd-opencode/command/gsd/settings.md
gsd-opencode/agents/gsd-settings.md
  </files>
...
# Legend line must be present in BOTH command + agents copies
rg -n "\* = overridden" gsd-opencode/command/gsd/settings.md
rg -n "\* = overridden" gsd-opencode/agents/gsd-settings.md

# Stronger static requirement: instruction text requires unconditional printing
rg -n "ALWAYS.*\* = overridden|Always.*\* = overridden" gsd-opencode/command/gsd/settings.md
```
