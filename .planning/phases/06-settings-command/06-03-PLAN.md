...
1) Create a fresh repo folder (not inside gsd-opencode):
   - `mkdir -p /tmp/gsd-phase06-test && cd /tmp/gsd-phase06-test && git init`
2) Install locally: `npx gsd-opencode --local`
3) Assert there are ZERO unresolved path tokens inside the install destination:
   - `rg -n "@gsd-opencode/|\\bgsd-opencode/" ./.opencode -S` should return **no matches**.
